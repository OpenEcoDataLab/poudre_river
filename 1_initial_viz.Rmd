...
oct <- map_dfr(files,weird_html_reader)
...
```{r}
full_seq <- tibble(datetime= seq(min(oct$datetime),
                                 max(oct$datetime),by=15*60))  %>%
  nest(data = everything())




full_unfold <- tibble(site=unique(oct$site),
                    full_time = rep(full_seq$data,length(site))) %>%
  unnest(full_time)

oct_clean <- oct %>%
  mutate(depth_lead = lead(depth,1),
         depth_rate = depth-depth_lead) %>%
